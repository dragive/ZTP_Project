...
public abstract class DataBaseRepository<T> {
        ...
        public T getByLogin(String login) {
            ...
            T ret = (T)query.getResultList().get(0);
            transaction.commit();
            session.close();
            return ret;
        }

        public T getByNIP(String NIP) {
            try{
                String hql = "FROM "+tClass.getName()+" E WHERE E.NIP LIKE '%"+NIP+"%'";
                Session session = sessionFactory.openSession();
                Transaction transaction =  session.beginTransaction();
                Query query= session.createQuery(hql,tClass);
                T ret = (T)query.getResultList().get(0);
...
session.close();
        ...
        public Long getNewId(){
            try {
                ...
                Long ret = (Long) query.getResultList().get(0)+1;
                transaction.commit();
                session.close();
                return ret;
            } catch (Exception ex) {
